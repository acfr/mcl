...
class WriteScreen(object):
    """Write network messages to the screen.

    The :py:class:`.WriteScreen` object is used for writing network data to the
    screen. This is useful for debugging the output of processes and monitoring
    network traffic.
    ...
    Args:
        ...
        screen_width (int): Maximum number of characters to print on the
                            screen.
        lock (threading.Lock): If set to None, no lock object will be used. If
                               a threading.Lock object is passed in,
                               :py:class:`.WriteScreen` will use the lock
                               during print operations. This prevents one
                               thread from interrupting a printing operation on
                               another thread (which results in merged lines).
    ...
    def __init__(self, format='human', column_width=None, screen_width=None,
                 lock=None):
        """Document the __init__ method at the class level."""

        # Store synchronisation object.
        self.__lock = lock

        # Validate printing format.
        self.__format = format
        if format not in ['raw', 'hex', 'human']:
            raise ValueError("Format must be 'raw', 'hex' or 'human'.")

        # Validate column width.
        self.__address_width = 10
        if not column_width or column_width >= 5:
            self.__column_width = column_width
        else:
            raise ValueError('Column width must be None or greater than 5.')

        # Validate screen width.
        self.__screen_width = screen_width
        if not screen_width or screen_width >= 40:
            self.__screen_width = screen_width
        else:
            raise ValueError('Screen width must be None or greater than 40.')

    def __truncate_string(self, string, width):
        """Truncate strings wider than a specified width.

        This function allows fixed with columns of text to be created by
        truncating long lines of text to the specified width. To indicate a
        truncation has occurred, the last three characters of the truncated
    ...
    def __format_raw(self, message):
        """Do not perform formatting - dump byte stream to screen"""

        return message['payload']

    def __format_hex(self, message):
        """Encode raw message into a hex string."""

        return message['payload'].encode('hex')
    ...
    def __format_human(self, message):
        ...
        # object is a dictionary and can be transmitted with additional
        # arbitrary items. ONLY print the the mandatory_items where the order
        # has been preserved.
        string = ''
        for item in message.mandatory_items():

            # Print float.
            if isinstance(message[item], float):
                data = '%5.3f' % message[item]

            # Print integer.
            elif isinstance(message[item], int):
                data = '%i' % message[item]

            # Print other values.
            else:
                data = '%s' % str(message[item])

            # Pad column with white space if it is shorter than the maximum
            # column width or delete the first three characters and prepend
            # with '...'
            if self.__column_width:
                if len(data) > self.__column_width:
                    start = 3 + len(data) - self.__column_width
                    data = '...' + data[start:]
                else:
                    data = data.rjust(self.__column_width)

            # No point writing items if the screen width has been exceeded.
            if self.__screen_width:
                if (len(string) + len(data)) > self.__screen_width:
                    break
    ...
    def __format_message(self, msg):
        """Format message as either 'raw', 'hex' or 'human'."""

        # Format network configuration
        name = self.__truncate_string(msg['name'], self.__address_width)
        address = self.__truncate_string(msg['address'], self.__address_width)
        topic = self.__truncate_string(msg['topic'], self.__address_width)

        # Print network configuration.
        string = '%s    %s    %s    '
        string = string % (name, address, topic)

        # Print unformatted raw data.
        if self.__format == 'raw':
            string += self.__format_raw(msg)

        # Print raw data as hex encoded.
        elif self.__format == 'hex':
            string += self.__format_hex(msg)

        # Print data as 'human' readable.
        elif self.__format == 'human':
            try:
                string += self.__format_human(msg)

            # If formatting fails. Print raw hex to screen.
            except:
                string += self.__format_hex(msg)

        # Control length of string.
        if self.__screen_width and (len(string) > self.__screen_width):
            string = string[0:self.__screen_width]

        return string

    def write(self, message):
        """Write network data to the screen.

        The :py:meth:`.WriteScreen.write` method writes network data to the
        screen. The output of data depends on the formatting option specified
        during instantiation. If 'raw' was chosen, data will be printed
        directly to the screen using the following format:

            <message name>    <address/URL>    <topic>    <raw data>

        If 'hex' was chosen, data will be printed to the screen using hex
        encoding and the following format:

            <message name>    <address/URL>    <topic>    <hex data>

        If 'human' was chosen, :py:meth:`.WriteScreen.write` will attempt to
        print the contents of the message to the screen using integers, floats
        and strings:

            <message name>    <address/URL>    <topic>    <field1>...<fieldN>

        If 'human' formatting was chosen and :py:meth:`.WriteScreen.write`
        cannot format the contents of the message, it will default to 'hex'
        formatting.

        :py:meth:`.WriteScreen.write` expects network data to be input as a
        ...
        where:
            ...
            - `topic` is the topic that was associated with the message
              broadcast
            - `object` is a pointer to the message object identified by the
              name field
            - `payload` is the network data received during transmission

        Args:
            message (dict): Network data to be printed to the screen. The
                            network data must be stored as a dictionary with
                            the name of the message object, the address (URL)
                            of the broadcast, the topic associated with the
                            broadcast, the message object and the message
                            payload as a MsgPack encoded string.

        """

        # Format message.
        string = self.__format_message(message)

        # Print raw/hex message to screen.
        if self.__lock:
            with self.__lock:
                print string
        else:
